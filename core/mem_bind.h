...
auto mem_bind(R (C::*ptmf)(FArgs...), Args&&... vs)
{
    ...
    return easy_bind::easy_bind(gen_seq<(sizeof...(FArgs) + 1) - sizeof...(Args)>(), ptmf, std::forward<Args>(vs)...);
}
...
auto mem_bind(T C::*ptmd, Args&&... vs)
{
    ...
    return easy_bind::easy_bind(gen_seq<1 - sizeof...(Args)>(), ptmd, std::forward<Args>(vs)...);
}
